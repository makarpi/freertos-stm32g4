...
int32_t I2C_Master_Write(void *handle, uint16_t slaveAddress, uint8_t reg, uint8_t *pBuffer, uint16_t length)
{
	...
	LL_I2C_HandleTransfer(handle, slaveAddress, LL_I2C_ADDRSLAVE_7BIT, length + 1, LL_I2C_MODE_AUTOEND, LL_I2C_GENERATE_START_WRITE);
  ...
  Timeout = I2C_SEND_TIMEOUT_TXIS_MS;
	...
	while (!LL_I2C_IsActiveFlag_STOP(handle)) {
		...
		if (LL_I2C_IsActiveFlag_TXIS(handle)) {
			...
			{
				LL_I2C_TransmitData8(handle, *(pBuffer++));
			}
...
{
	LL_I2C_HandleTransfer(handle, slaveAddress, LL_I2C_ADDRSLAVE_7BIT, 1, LL_I2C_MODE_AUTOEND,
	...
	LL_I2C_HandleTransfer(handle, slaveAddress, LL_I2C_ADDRSLAVE_7BIT, length, LL_I2C_MODE_AUTOEND, LL_I2C_GENERATE_START_READ);
	...
	while (!LL_I2C_IsActiveFlag_STOP(handle)) // Loop until end of transfer received (STOP flag raised)
	{
		...
		if (LL_I2C_IsActiveFlag_RXNE(handle))
		{
			...
			*(pBuffer++) = LL_I2C_ReceiveData8(handle);
		}
	}
